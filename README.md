...
# Documents
There are two Excel documents: `driving.csv` and `straightline.csv`. The `driving.csv` records the actual distances between two states. If the data is -1, that means there is no path between two states. The `straightline.csv` file contains the straight-line distances between two states. Greedy best-first search only uses the `driving.csv` file, but A* applies both files.
